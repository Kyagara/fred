...
public class MusicPlayerScreen extends BaseOwoScreen<FlowLayout> {
	...
	protected void build(FlowLayout rootComponent) {
		...
		for (Identifier song : songs) {
			songsContainer.child((Component) Components.button(Text.translatable(song.toString()), button -> {
				if (client == null) {
					return;
				}
		...
		for (Identifier category : categories) {
			categoriesContainer.child((Component) Components.button(Text.translatable(category.toString()), button -> {
				if (client == null) {
					return;
				}
